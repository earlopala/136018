...
<head>
    <meta charset="utf-8">
    <title>Cholera Diagnostics System - Administrator Homepage</title>
...
$sql = "SELECT `Symptom_ID`, `User_ID`, `Details`, `Status`, `Created_At` FROM `symptoms`";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
$sql = "SELECT `diagnostic`.`Diagnostic_ID`, `diagnostic`.`Symptom_ID`, `diagnostic`.`Results`, `diagnostic`.`File`, `diagnostic`.`Created_At`, `symptoms`.`Status` FROM `diagnostic` JOIN `symptoms` ON `symptoms`.`Symptom_ID` = `diagnostic`.`Symptom_ID`";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
?>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to confirm this diagnostic (as well as symptom record)?')?window.location.href='actions.php?action=updateSD&id=<?php echo($row["Symptom_ID"]); ?>':true;" title='Confirm Symptom & Diagnostic'>Confirm</button></td>
<?php
}else{
?>
<td>Diagnosis is already Confirmed.</td>
<?php
}
?>
</tr>
        ...
        <div class="container">
            <div class="row g-5">
                <div class="col-lg-6 wow fadeInUp" data-wow-delay="0.1s">
                    ...
                    <h1 class="mb-4">Update My Details and Register a User</h1>
                    ...
                    <div class="bg-light rounded h-100 d-flex align-items-center p-5">
                        <form action="actions.php" method="POST">
                            <div class="row g-3">
                                ...
                                <div class="col-12 col-sm-6">
                                    <select class="form-select border-0" required name="type" style="height: 55px;">
                                ...
                                <div class="col-12">
                                    <button class="btn btn-primary w-100 py-3" type="submit" name="regu">Register</button>
                                </div>
                            </div>
                        </form>
                    </div>
